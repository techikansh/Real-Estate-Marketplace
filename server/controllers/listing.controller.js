...
export async function deleteUserListing(req, res, next) {
    ...
    if (id != listing.userRef) return next(errorHandler(403, "Sie können nur Ihre eigene Daten löschen"));
...
export async function updateUserListing (req, res, next) {
    ...
    if (id != listing.userRef) return next(errorHandler(403, "Sie können nur Ihre eigene Daten aktualisieren"));
